...
namespace Identity.Dapper.Tests.Queries.MySQL
{
    public class MySqlUserQueriesTests
    {
        ...
        public void GetUserLoginByLoginProviderAndProviderKeyQueryGeneratesCorrectQuery()
        {
            ...
            const string expected = "SELECT `identityuser`.`AccessFailedCount`, `identityuser`.`Email`, `identityuser`.`EmailConfirmed`, `identityuser`.`LockoutEnabled`, `identityuser`.`LockoutEnd`, `identityuser`.`PasswordHash`, `identityuser`.`PhoneNumber`, `identityuser`.`PhoneNumberConfirmed`, `identityuser`.`SecurityStamp`, `identityuser`.`TwoFactorEnabled`, `identityuser`.`UserName`, `identity`.`identityuserrole`.* FROM `identity`.`identityuser` LEFT JOIN `identity`.`identityuserrole` ON `identity`.`identityuserrole`.`UserId` = `identity`.`identityuser`.`Id` INNER JOIN `identity`.`identitylogin` ON `identity`.`identityuser`.`Id` = `identity`.`identitylogin`.`UserId` WHERE `LoginProvider` = @LoginProvider AND `ProviderKey` = @ProviderKey LIMIT 1";
        ...
        public void GetUserLoginByLoginProviderAndProviderKeyQueryGeneratesCorrectQueryWhenUsingCustomUser()
        {
            ...
            const string expected = "SELECT `identityuser`.`AccessFailedCount`, `identityuser`.`Dummy`, `identityuser`.`Email`, `identityuser`.`EmailConfirmed`, `identityuser`.`LockoutEnabled`, `identityuser`.`LockoutEnd`, `identityuser`.`PasswordHash`, `identityuser`.`PhoneNumber`, `identityuser`.`PhoneNumberConfirmed`, `identityuser`.`SecurityStamp`, `identityuser`.`TwoFactorEnabled`, `identityuser`.`UserName`, `identity`.`identityuserrole`.* FROM `identity`.`identityuser` LEFT JOIN `identity`.`identityuserrole` ON `identity`.`identityuserrole`.`UserId` = `identity`.`identityuser`.`Id` INNER JOIN `identity`.`identitylogin` ON `identity`.`identityuser`.`Id` = `identity`.`identitylogin`.`UserId` WHERE `LoginProvider` = @LoginProvider AND `ProviderKey` = @ProviderKey LIMIT 1";
